...
from litepanel.api.auth import api_auth
from litepanel.models import Website, User, FTPAccount, Database, GitRepo, SSLCertificate, APIToken
...
def websites(request):
    ...
    from litepanel.admin_views.websites import create_website as _cw
    ...
    try:
        ...
        site.full_clean()
        site.save()
        from litepanel.services.ols import create_docroot, write_vhost_config, reload_ols
...
def website_detail(request, site_id):
    ...
    if request.method == 'DELETE':
        if request.api_user.role != 'admin':
            return _err('Admin only', 'FORBIDDEN', 403)
        from litepanel.services.ols import delete_vhost_config, reload_ols
    ...
    if request.method == 'PUT':
        ...
        from litepanel.services.ols import write_vhost_config, reload_ols
...
def ftp_accounts(request):
    ...
    if not _owns(request.api_user, site):
        return _err('Forbidden', 'FORBIDDEN', 403)
    from litepanel.services.ftp import create_ftp_account
    try:
...
def git_repos(request):
    ...
    if not _owns(request.api_user, site):
        return _err('Forbidden', 'FORBIDDEN', 403)
    from litepanel.services.git import clone_repo
    try:
...
def databases(request):
    ...
    if not _owns(request.api_user, site):
        return _err('Forbidden', 'FORBIDDEN', 403)
    from litepanel.services.db_manager import create_database
    try:
